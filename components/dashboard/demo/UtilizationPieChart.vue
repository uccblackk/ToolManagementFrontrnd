...
export default {
  ...
  methods: {
    ...
    transData() {
      ...
      let statusList = [
        { status: "加工中", color: "#01B468" },
        { status: "異常", color: "red" },
        { status: "閒置", color: "#FFBB77" },
        { status: "斷線", color: "gray" }
      ];
      let restCount = 0;
      let count = this.machineList.length;
      statusList.forEach(s => {
        let value = Math.random();
        let ram = this.GetRandomInt(0, this.machineList.length - restCount);
        count = this.machineList.length - restCount - ram;
        //console.log(ram);
        restCount += count;
        // if (restCount > this.machineList.length) {
        //   count = ;
        // }
      ...
      this.timerId = setTimeout(this.transData, 1000 * 5);
    },
