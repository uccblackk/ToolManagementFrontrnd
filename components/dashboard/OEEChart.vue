...
export default {
  ...
  methods: {
    ...
    async getOEEData() {
      ...
      this.$axios
        .$get(
          `/OEE/GetTotalMachineOEE?startTime=${dates[0]}&endTime=${dates[0]}&areaNo=${this.areaNo}`,
          {
            headers: {
              Authorization: token
            }
        ...
        .then(res => {
          OEEData = res.data;
          if (OEEData.utilization > 1) {
            OEEData.utilization = 1;
          }
          this.utilization = OEEData.utilization * 100;
          if (OEEData.performance > 1) {
            OEEData.performance = 1;
          }
