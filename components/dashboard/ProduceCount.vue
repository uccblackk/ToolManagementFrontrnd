...
export default {
  ...
  methods: {
    ...
    async getOEEData() {
      ...
      this.$axios
        .$get(
          `/OEE/GetOEECount?startTime=${dates[0]}&endTime=${dates[0]}&areaNo=${this.areaNo}`,
          {
            headers: {
              Authorization: token
            }
        ...
        .then(res => {
          OEEData = res.data;
          this.reportCount = OEEData.reportCount;
          this.cycleCount = OEEData.cycleCount;
          ...
          this.loading = false;
          this.timerId = setTimeout(this.getOEEData, 10000);
        })
        .catch(({ response }) => {
          this.loading = false;
          this.timerId = setTimeout(this.getOEEData, 10000);
