<template>
  <v-layout row wrap style="width:100%">
    ...
    <v-dialog v-model="jobReportDialog" persistent max-width="300px">
      <v-card ref="form">
        <v-card-title :class="selectedDialogColor">
        ...
        <v-card-actions>
          ...
          <v-btn :class="selectedDialogColor" dark text @click="jobReport()">
            {{ $t("confirm") }}
          </v-btn>
        </v-card-actions>
      </v-card>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {
          text: this.$t("next") + this.$t("maintenance") + this.$t("time"),
          value: "planEndTime",
          align: "left",
          sortable: true,
          width: "150px"
  ...
  methods: {
    ...
    showJobReportDialog(item) {
      if (item.status == "N") {
        this.selectedDialogColor = "success";
        ...
        this.selectedDialogColor = "primary";
        this.jobReportDialogText =
          this.$t("confirm") +
          this.$t("completed") +
          this.$t("equipmentMaintenance") +
    ...
    updateStatus(status) {
      ...
      const data = {
        ...
        machineId: this.selectedItem.machine.id,
    ...
    edit(item) {
      ...
      this.editForm.machine = item.machine;
      this.areaData = this.areaList.find(x => x.areaNo == item.machine.areaNo);
    },
    editData() {
      if (
        ...
        this.editForm.machine == undefined
      ) {
      ...
      const data = {
        creatorId: this.$store.state.authUser.id,
        id: this.editForm.id,
        machineId: this.editForm.machine.id,
    ...
    addData() {
      if (
        ...
        this.addForm.machine == undefined
      ) {
      ...
      const data = {
        creatorId: this.$store.state.authUser.id,
        machineId: this.addForm.machine.id,
    ...
    showAddDialog() {
      ...
      this.addForm.machine = this.machineList[0];
    }
  },
  mounted() {
    this.getUserData();
    ...
    areaData() {
      ...
      this.$axios
        ...
        .then(res => {
          ...
          this.addForm.machine = this.machineList[0];
          this.editForm.machine = this.machineList[0];
          // console.log(this.addForm.machine);
        });
    }
  },
  created() {
