  ...
  <v-layout row wrap>
    <v-flex md12 class="pa-5 mt-n10">
      ...
      >
        ...
        <template v-slot:item.rootArea="{ item }">
          <v-chip color="blue lighten-5" v-if="item.rootArea != undefined">
            {{ item.rootArea.areaName + " (" + item.rootArea.areaNo + ")" }}
          </v-chip>
        </template>
      </v-data-table>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {
          text: this.$t("upperLevel") + this.$t("area"),
          value: `rootArea`,
          sortable: false,
          width: "10%"
        },
  ...
  methods: {
    editItem(item) {
      ...
      this.selectedArea = item.rootArea;
      this.selectAreaList = this.areaList.slice(0);

      this.selectAreaList.unshift({ areaName: "無", id: null });
      console.log(this.selectAreaList.indexOf(item));
      this.selectAreaList.splice(this.selectAreaList.indexOf(item), 1);
