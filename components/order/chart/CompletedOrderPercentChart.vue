  ...
  <v-layout row wrap style="width: 100%">
    <v-flex xs12 class="pa-2 mt-2">
      <div style="width: 100%; height: auto">
        <div
          id="CompletedOrderPercentChart"
          style="width: 100%; height: 600px"
          ref="CompletedOrderPercentChart"
        ></div>
      </div>
    </v-flex>
  </v-layout>
  ...
  methods: {
    ...
    transData() {
      ...
      this.data.forEach(lotOp => {
        doneCount = lotOp.doneData[this.opData.opNo];
        totalCount = lotOp.totalLotOpCount;
        this.doneDataPoints.push({
          y: doneCount,
    ...
    renderData() {
      ...
      if (this.chart == null) {
        this.chart = new CanvasJS.Chart("CompletedOrderPercentChart", {
          ...
          subtitles: [
            {
              text: this.$t("completedOrderPercent"),
              fontSize: 16
            }
          ],
          axisX: {
          ...
          data: [
            {
              ...
              showInLegend: true,
              color: "orange",
              name: this.$t("completed") + this.$t("count"),
              dataPoints: this.doneDataPoints
            },
            {
              type: "line",
              showInLegend: true,
        ...
        this.chart.options.subtitles[0].text = this.$t("completedOrderPercent");
        this.chart.options.data[0].dataPoints = this.doneDataPoints;
